---
title: "A Corpus-Based Acoustic Analysis of Monophthong Vowels among Chinese Learners and Native Speakers of English  - Part 2"
...
fdat %>%
  ...
  ggplot(aes(y = word, x = label, fill = freq)) +
  geom_tile() +
  theme_bw()
```
...
fdat <- fdat %>%
  ...
  dplyr::mutate(remove = dplyr::case_when(word == "--we"  ~ "remove",
                                          ...
                                          word == "we're" & label == "6" ~ "remove",
                                          word == "gas" & label == "I" ~ "remove",
                                          vowel == "ɐ" ~ "remove",
                                          stringr::str_detect(word, "\\W") ~ "remove",
                                          TRUE ~ "keep")) %>%
  dplyr::filter(remove != "remove")
# inspect
table(fdat$label, fdat$word)[1:6,]
table(fdat$label, fdat$word)[7:11,]
```
...
Schweinberger, Martin. `r format(Sys.time(), '%Y')`. A Corpus-Based Acoustic Analysis of Monophthong Vowels among Chinese Learners and Native Speakers of English. Brisbane: The University of Queensland, School of Languages and Cultures. 
